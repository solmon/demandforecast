...
export function PieChart({
  ...
  useEffect(() => {
    ...
    // Arc generators
    const arc = d3.arc<d3.PieArcDatum<any>>().innerRadius(0).outerRadius(radius).cornerRadius(4);

    // Create arcs
    const pieData = pie(sortedData);
    const arcs = svg.selectAll('.arc').data(pieData).enter().append('g').attr('class', 'arc');
    ...
    slicesToLabel.forEach((d) => {
      ...
      // Add name
      text.append('tspan').attr('x', 0).attr('dy', '0em').text(d.data[nameKey]);
      ...
      // Apply transition
      text.transition().delay(800).duration(500).style('opacity', 1);
    });
    ...
    if (smallSlices.length > 0) {
      ...
      smallSlices.forEach((d, i) => {
        ...
        legendGroup.append('rect').attr('width', 10).attr('height', 10).attr('fill', colorScale(d.data[nameKey]));
