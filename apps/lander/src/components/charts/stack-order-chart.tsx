...
export function StackOrderChart({
  ...
  useEffect(() => {
    ...
    // Color scale
    const color = d3.scaleOrdinal().domain(keys).range(colors.slice(0, keys.length));
    ...
    overlay.on('mousemove', function (event) {
      ...
      const d =
        xDate.getTime() - (d0[xKey] as Date).getTime() > (d1[xKey] as Date).getTime() - xDate.getTime() ? d1 : d0;
